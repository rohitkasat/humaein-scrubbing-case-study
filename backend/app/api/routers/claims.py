...
@router.get("/audit")
async def audit_claims(db: Session = Depends(get_db)):
...
# Audit endpoint: returns all claims with audit fields
@router.get("/audit")
async def audit_claims(db: Session = Depends(get_db)):
    claims = db.query(Claim).all()
    # Return all claims with audit fields
    return {
        "claims": [
            {
                "id": c.id,
...
async def list_claims(db: Session = Depends(get_db)):
    """
    Returns all claims. The master table (claims) serves as both the raw and refined table.
    The 'stage' field indicates whether a claim is 'raw' (uploaded, not validated) or 'refined' (validated/processed).
    """
    claims = db.query(Claim).all()
    return {
        "claims": [
